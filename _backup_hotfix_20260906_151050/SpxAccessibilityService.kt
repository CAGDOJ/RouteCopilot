...
class SpxAccessibilityService :
    ...
    private fun scanCurrentScreen() {
        ...
        // ------------------------------------------------
        // 1º: LOCALIZAR A SETA / BOTÃO DE DOWNLOAD
        // ------------------------------------------------
        if (!downloadClicked) {
            update(
            ...
            if (
                ...
                failAndReturn(
                    "Não encontrei a seta de download da rota no SPX."
                )
                return
            }
        ...
        // ------------------------------------------------
        // 2º: APÓS DOWNLOAD, IR PARA ENTREGA / EM ROTA.
        //
        // REGRA CRÍTICA:
        // NUNCA aceitar só porque o texto "Em Rota" existe
        ...
        if (!inRouteClicked) {
            ...
            if (
                ...
                failAndReturn(
                    "Não consegui abrir a aba Em Rota. A sincronização foi cancelada."
                )
                return
            }
        ...
        // ------------------------------------------------
        // 3º: SOMENTE DEPOIS DO CLIQUE EM "EM ROTA"
        // LER AT, TOTAL E BRs.
        // ------------------------------------------------
        update(
    ...
    // ----------------------------------------------------
    // BANNER PEQUENO E NÃO-BLOQUEANTE.
    //
    // Não cobre a tela inteira, não captura toques e não
    // impede Voltar/Home.
    // ----------------------------------------------------
